...
public abstract class Publish_Tests : SellableItemEndpoints_Tests
{
    protected static readonly Guid TestId = Guid.NewGuid();

    public class When_publishing_successfully : Publish_Tests
    {
        protected override void Arrange()
        {
            MockOf<ISellableItemCommandService>()
                .PublishAsync(TestId, Arg.Any<CancellationToken>())
        ...
        protected override async Task Act()
            => Output = await Execute(SellableItemEndpoints.Publish, TestId);
    ...
    public class When_service_throws_exception : Publish_Tests
    {
        protected override void Arrange()
        {
            MockOf<ISellableItemCommandService>()
